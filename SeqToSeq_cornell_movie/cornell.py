from dataset.cornell_data import cleanup_sentence, process_cornell
from utilities import batches_dataset, process_batch_data, UNK, GO, EOS
from models import Config, SeqToSeq
...
def model_play(mode):
    ...
    if not os.path.exists(tf_config["save_dir"]) or not os.listdir(tf_config["save_dir"]):
        sys.stdout.write("No preprocessed dataset found, create from {} raw data...\n"
                         .format(tf_config["save_dir"]))
        sys.stdout.flush()
        process_cornell(tf_config)
    sys.stdout.write("Loading Configurations....\n")
    sys.stdout.flush()
    ...
    if mode == "train":
        ...
        sys.stdout.flush()
        train_batches, test_batches = batches_dataset(tf_config["dataset"], config.batch_size, config.target_dict)
        # build model and start training
        sys.stdout.write("Building model...\n")
        sys.stdout.flush()
        seq2seq_model = SeqToSeq(config, mode="train")
        seq2seq_model.train(train_batches, test_batches, epochs=config.epochs)
    elif mode == "decode":
        sys.stdout.write("Building Model....\n")
        sys.stdout.flush()
        seq2seq_decode_model = SeqToSeq(config, mode="decode")
        seq2seq_decode_model.restore_last_session()
        sys.stdout.write("> ")
        sys.stdout.flush()
        top_n = False  # if beam search, return all decoded results or just the first one
        sentence = sys.stdin.readline()
        sentence_ids = sentence_to_ids(sentence, config.target_dict)
        while sentence:
            predict_ids = seq2seq_decode_model.inference(sentence_ids)
            response = ids_to_sentence(predict_ids, config.rev_target_dict, config.target_dict)
            print(response)
            if top_n:
                print("\n".join(response))
            else:
